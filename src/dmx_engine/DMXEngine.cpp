...
void DMXEngine::runOutput()
{
    ...
    {
        std::chrono::milliseconds startMs = duration_cast<milliseconds>(
        ...
        for (int i = 0; i < this->universeNum; i++)
        {
            ...
            {
                std::shared_ptr<Output> output = this->dmxOutputs.at(i);
        ...
        std::chrono::milliseconds endMs = duration_cast<milliseconds>(
            system_clock::now().time_since_epoch()
        );

        this->averageOutputFps = smoothingValue * this->averageOutputFps + (1.0 - smoothingValue) * (1/((endMs.count() - startMs.count()) / 1000.0));
    }
...
void DMXEngine::runEffects()
{
    ...
    while (!this->shouldThreadStop)
    {
        ...
        this->averageEffectFps = smoothingValue * this->averageEffectFps + (1.0 - smoothingValue) * (1/((endMs.count() - startMs.count()) / 1000.0));
    }
